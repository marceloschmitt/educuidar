...
function normalizeCpf(value) {
    ...
    return String(value).replace(/\D+/g, '').substring(0, 11);
}
...
function getAlunoCpf(aluno) {
    if (!aluno) return '';
    // Aceita cpf / CPF e garante string (evita perda de zeros à esquerda)
    var raw = aluno.cpf !== undefined ? aluno.cpf : aluno.CPF;
...
function resetForm() {
    ...
    var modalCpf = document.getElementById('modal_cpf');
    if (modalCpf) modalCpf.value = '';
...
function viewFichaAluno(aluno) {
    ...
    var fichaCpf = document.getElementById('ficha_cpf');
    if (fichaCpf) {
        var cpfFmt = formatCpf(getAlunoCpf(aluno));
        fichaCpf.textContent = cpfFmt || '-';
    }
...
function editAluno(aluno) {
    ...
    var modalCpfEdit = document.getElementById('modal_cpf');
    if (modalCpfEdit) modalCpfEdit.value = formatCpf(getAlunoCpf(aluno));
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (modalCpfInput) {
        modalCpfInput.addEventListener('input', function() {
            this.value = formatCpf(this.value);
        });
        modalCpfInput.addEventListener('blur', function() {
            this.value = formatCpf(this.value);
        });
    }
